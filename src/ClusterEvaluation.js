...
export function ClusterEvaluation(){
	/*
	...
	this.WSS = function(){
		...
		data.forEach(function(d){
			...
			if(centroid === null || centroid === undefined){
				sse = Evaluation().data(points).accessor(point_accessor).SSE();
			}
			else{
				sse = Evaluation().data(points).accessor(point_accessor).SSE(centroid_accessor(d));
			}
